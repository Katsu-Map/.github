...
* 커뮤니티 카드 본문 미리보기 및 더보기 추가
* 피드 카드 아이콘 순서 변경
* 피드에서 가게명 클릭 시 지도로 이동
* 사장님의 한마디 상시 노출
* 커뮤니티 탭 마스코트 헤더 문구 추가
...
## [ V 2.2.2 ] - 검색 개편 및 미등록 가게 기록 플로우 개선 🔍
...
* 검색 목적별 API 분리
  ...
  * 지역 검색
  * 장소 검색
* 장소 검색 기준 반경 1km 내 맛집 조회 기능 추가
* 검색 결과 및 검색 UX 개선

#### 미등록 가게 기록 플로우 추가

* DB에 등록되지 않은 가게도 기록 작성 가능하도록 변경
* 미등록 가게 기록 작성 시 나만보기 상태로 저장
* 미등록 가게 기록 작성 완료 안내 팝업 추가
* Admin 내 미등록 가게 기록 조회 기능 추가
* 관리자 가게 등록 및 기존 기록 연결 기능 추가
* 관리자 승인 후 커뮤니티 피드 자동 공개
* 승인 완료 시 작성자 Push Notification 발송
* 알림 클릭 시 커뮤니티 탭 이동 처리

#### 커뮤니티 및 기록 기능 개선

* 커뮤니티 피드 이미지 사이즈 확대
* 피드 내 다중 이미지 확인 UX 개선
* 피드에서 해당 가게 바로 즐겨찾기 기능 추가
* 기록 작성 화면 이미지 영역 확대
* 기록 작성 텍스트 입력 영역 확대
* 제보자 한마디 및 콘텐츠 활용 안내 추가
* 본인 댓글에 대한 대댓글 작성 허용
* 댓글 하단 답글 작성 UI 추가
* 피드 생성 / 수정 후 Feed Cache 최신화
* 기록 작성 및 수정 직후 피드에 반영되지 않던 문제 수정

#### 가게 데이터 구조 개선

* 가게별 최대 5개 태그 저장 구조 추가
* 자유 입력 방식이 아닌 Tag Master 기반 구조 적용
* Admin 페이지 내 가게 태그 관리 기능 추가
* 추후 검색 / 추천 / 필터링 확장을 고려한 태그 구조 구성

#### 데이터 집계 및 로깅 개선

* 개인정보 최소 수집을 위해 사용자별 상세 이벤트 로깅 제거
* 서비스 개선을 위한 Aggregate Logging 구조 적용
* 주요 탭별 조회수 집계
* 백과사전 게시글 조회수 집계
* 카츠헌터 콘텐츠 조회수 집계
* 검색 이용량 집계
* 접속 지역 단위 집계
* 개별 사용자를 특정하거나 개인별 행동 이력을 추적하지 않는 통계 중심 구조 적용
* 추후 검색 / 추천 / UX 데이터 분석에 활용할 수 있도록 집계 데이터 구조화
...
* 가격대 표기 방식 변경
  ...
  * `$$` 형식 → `평균 1만원대` 형식
* 홈 지도 Bottom Sheet 최대 높이 및 인터랙션 개선
* 내 취향 맛집 카드 지역명 말줄임 / 잘림 현상 수정
* 별점 분포에 따른 사용자 평가 성향 코멘트 추가
* 일부 페이지 Navigation 및 화면 흐름 개선

#### 버그 및 안정성 개선

* 피드 등록 및 수정 후 즉시 최신화되지 않던 문제 수정
* 일부 레이아웃 및 텍스트 Overflow 문제 수정
* 기타 마이너 버그 및 UI 수정
